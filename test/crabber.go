...
func main() {
	// Сохраняем текущее время в начале программы
	start := time.Now()
	...
	srcPath := flag.String("src", "", "Path to the file with URLs")
	dstPath := flag.String("dst", "", "Path to the parsed site")
	// Создание флагов
	flag.Parse()

	// Проверяет, были ли заданы значения флагов --src и --dst, если нет, то print и завершение программы
	if *srcPath == "" || *dstPath == "" {
		fmt.Println("Usage: ./crabber --src=source.txt --dst=destination ")
		return
	}
	...
	if err != nil {
		fmt.Printf("Error creating destination directory: %s", err)
		return
	}
...
func treatmentURL(url string, dstPath string) error {
	fmt.Printf("URL processing: %s", url)

	// Выполняет HTTP GET запрос к указанному URL
	resp, err := http.Get(url)
	if err != nil {
		fmt.Errorf("Error fetching URL %s", err)
	}
	...
	// Считывает содержимое тела ответа и записывает его в файл
